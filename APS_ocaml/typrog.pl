...
/* expr -> abs */
type_expr(G,abs(ListArg,E),fleche(ListT,T2)):-env_extend(G,ListArg,G2),type_expr(G2,E,T2).
env_extend(G,[],G).
...
/* def -> fun */
type_def(G,fun(X,T,ListArg,E),[X,fleche((ListT,T)|G)]):-env_extend(G,ListArg,G2),type_expr(G2,E,T).
/* def -> fun rec */
type_def(G,funrec(X,T,ListArg,E),[X,fleche((ListT,T)|G)]):-env_extend(G,ListArg,G2),type_expr([(X,fleche(ListT,T))|G2],E,T).
